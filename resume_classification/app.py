...
con.execute("create table if not exists employerfile(pid integer primary key,pdf TEXT)")
con.close()
...
def employer_upload():
    ...
    cur = con.cursor()
    cur.execute("select * from employerfile")
    data = cur.fetchall()
    con.close()
    ...
    if request.method == 'POST':
        ...
        if upload_pdf.filename!='':
            ...
            con = sqlite3.connect("MyPDF.db")
            cur = con.cursor()
            cur.execute("insert into employerfile(pdf)values(?)", (upload_pdf.filename,))
            con.commit()
...
def delete_record(id):
    try:
        ...
        return redirect(url_for("employer_upload"))
        con.close()

@app.route('/match/<string:id>', methods=("POST","GET"))
def match(id):
    con = sqlite3.connect("MyPDF.db")
    con.row_factory = sqlite3.Row
    cur = con.cursor()
    ...
    if request.method == 'POST':
        try:
            upload_pdf = request.files['upload_PDF']
            refile = os.path.join(app.config['UPLOAD_FOLDER'], upload_pdf.filename)
            upload_pdf.save(refile)
